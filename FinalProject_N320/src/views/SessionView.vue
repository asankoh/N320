...
methods:{
  ...
  addSession(value){
    // let sessionContent = this.sessions.filter(session => session.title.includes(this.title))
    let sessionContent = this.sessions.filter((session) =>{
    return session.title == value
  })
   this.mySessions.push(sessionContent)
    console.log("Session Content:", this.mySessions)
  }
    },
    computed:{
      sessionAmount(){
